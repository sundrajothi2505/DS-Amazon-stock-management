...
from db import get_db, close_db, init_db, fetch_products, fetch_product, TYPE_LABELS, ORDER_STATUS_LABELS
...
def product_import():
    if request.method == "POST":
        ...
        for row in rows:
            ...
            if details_changed:
                conn.execute(
                    "UPDATE products SET name=?, asin=?, updated_at=datetime('now') WHERE id=?",
...
def orders():
    ...
    product_list = conn.execute(
        "SELECT id, sku, name FROM products ORDER BY name COLLATE NOCASE"
    ).fetchall()
    ...
    if request.method == "POST":
        ...
        conn.execute(
            """UPDATE orders SET order_date=?, order_number=?, product_id=?, product_description=?,
               quantity=?, status=?, notes=?, updated_at=datetime('now') WHERE id=?""",
...
def users():
    conn = get_db()
    rows = conn.execute("SELECT * FROM users ORDER BY full_name COLLATE NOCASE").fetchall()
